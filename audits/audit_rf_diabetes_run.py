#!/usr/bin/env python3
"""Program 2b audit #19 runner: Breiman (2001) Random Forests, Table 2, diabetes row (Forest-RI).
...
Pinned per the pre-registration (audits/AUDIT_breiman2001-rf-diabetes.md, commit 37200fe):
...
Data: OpenML id 37 ARFF (Pima), md5-asserted; tested_negative=0, tested_positive=1.
Chunkable: any [start,end) iteration range reproduces identically.

Usage:
    python3 audit_rf_diabetes_run.py run <master_seed> <iter_start> <iter_end> <out_json>
    python3 audit_rf_diabetes_run.py merge <out_raw_json>   # merge /tmp/rfdiab_m*_*.json partials
"""
...
DATA = "/tmp/diabetes.arff"
PUB = {"single_input": 24.3, "selection": 24.2}

def load():
    raw = open(DATA, "rb").read()
    md5 = hashlib.md5(raw).hexdigest()
    assert md5 == "3cbaa3e54586aa88cf6aacb4033e4470", md5
    ...
    y = np.array([0 if r[8].strip() == "tested_negative" else 1 for r in rows])
    assert X.shape == (768, 8) and int(y.sum()) == 268 and len(rows) == 768
    return X, y, md5
...
def merge(out_raw):
    import sklearn
    seeds = {}
    for f in sorted(glob.glob("/tmp/rfdiab_m*_*.json")):
        d = json.load(open(f))
