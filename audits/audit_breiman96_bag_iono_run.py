#!/usr/bin/env python3
"""Program 2b audit #22 runner: Breiman (1996) Bagging Predictors, Table 2, ionosphere rows.

Dataset swap of the audit #14 runner (audits/audit_breiman96_bag_glass_run.py); the
...
min-error selection with ties -> larger alpha, unstratified split, |test|=35, |L_B|=316,
classes encoded by np.unique ('b' < 'g').

Usage: audit_breiman96_bag_iono_run.py <master_seed> <iter_start> <iter_end> <out_json>
"""
...
def load(path):
    ...
    y = np.array([r[34] for r in rows])  # 'b' / 'g'; np.unique -> 'b' < 'g'
    return X, y, md5
...
if __name__ == "__main__":
    ...
    X, y, md5 = load(DATA)
    assert md5 == MD5, md5
    # NOTE (disclosed in the audit's honesty section): the paper's Appendix states
    # good 226 / bad 125, but the md5-pinned UCI file (same file as audits #4/#16)
    # contains 225 'g' / 126 'b' - a one-case labeling difference in the archived copy.
    assert X.shape == (351, 34) and (y == "g").sum() == 225 and (y == "b").sum() == 126
