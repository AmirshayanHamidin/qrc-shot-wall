#!/usr/bin/env python3
"""Program 2b audit #14 runner: Breiman (1996) Bagging Predictors, Table 2, glass rows.

Reproduces e_S (single CART, 10-fold-CV pruned) and e_B (bagging, 50 bootstrap trees,
pruned against the original learning set L per Sec. 4.3, plurality vote, ties -> lowest
class label). 100 iterations of a random 90/10 split; means over iterations.
...
min-error selection with ties -> larger alpha, unstratified split, |test|=21, |L_B|=193.

Usage: audit_breiman96_bag_glass_run.py <master_seed> <iter_start> <iter_end> <out_json>
"""
...
N_BOOT = 50
N_FOLDS = 10
N_TEST = 21  # round(0.1*214)

def load(path):
    raw = open(path, "rb").read()
    md5 = hashlib.md5(raw).hexdigest()
    rows = [l.split(",") for l in raw.decode().strip().split("\n")]
    X = np.array([[float(v) for v in r[1:10]] for r in rows])
