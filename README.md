...
**Task dependence (B3).** The wall is a property of output precision, not of the device. On temporal parity-3 — where a linear model on the inputs is provably at chance — the stored benchmark reports the same reservoir at the same 40k-shot budget reaching 93% accuracy and retaining 86% of its exact-readout benefit, while NARMA regression at the same budget retains ~4% ([`results/RESULTS_TASKSHAPE.md`](results/RESULTS_TASKSHAPE.md)). A pre-registered reconstruction from committed code ([`audits/PREREG_B3_GEN.md`](audits/PREREG_B3_GEN.md), raw output in `results/task_shape_recon.json`) supports this qualitative contrast — the reconstructed curve retains essentially all of the benefit over the linear baseline at 40k shots — but does not reproduce the stored values: the reconstructed curve runs above the stored one at every budget, and the reconstructed tuned ESN reference solves parity-3 outright (1.00 against the stored 0.77), so the earlier report of a crossing of the ESN near 12k shots/step is not supported under the registered protocol, and the stored raw file keeps its provenance-lost annotation ([`AUDITS.md`](AUDITS.md)). Coarse-output tasks — classification, detection, decision — sit below the wall; precise trajectory regression sits above it.
...
Published numbers are re-executed from the committed code in recurring audit passes, logged in [`AUDITS.md`](AUDITS.md) with reproduced and published values side by side. Most benchmark outputs regenerate byte-identically. Where an audit found a published number it could not regenerate — the original B5 headline precision, two B2 table rows, and a B2 redundancy probe — the finding is disclosed in the audit log, and the number was subsequently either restored from a committed pinned-convention generator or corrected in place. The B3 accuracy curve was reconstructed under a pre-registered protocol (2026-08-02): the reconstruction supports the benchmark's qualitative claim but does not land on the stored curve or its tuned-ESN reference, which therefore retain a provenance-lost annotation in the audit log. A consolidated draft write-up is in [`PREPRINT.md`](PREPRINT.md), with literature positioning in [`RELATED_WORK.md`](RELATED_WORK.md).
