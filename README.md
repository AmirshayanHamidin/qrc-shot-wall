...
Two task families now tested (binary parity/NARMA and continuous Mackey-Glass; B11), one reservoir topology, sampling noise only — real hardware adds gate errors, so the wall here is *optimistic*. Feature-count-matched (not wall-clock-matched) classical comparisons. Details and seeds in the write-ups.
