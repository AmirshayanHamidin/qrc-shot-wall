...
5. **Want the whole arc in one document?** [`PREPRINT.md`](PREPRINT.md) — the consolidated arXiv skeleton: abstract, 13-benchmark narrative, law derivation sketch, honest-negative ledger, limitations — with a related-work positioning pass in [`RELATED_WORK.md`](RELATED_WORK.md). Independent verification entries live in [`AUDITS.md`](AUDITS.md): B13 reproduced end-to-end from independent code (CONFIRMED); B5's headline precision found **not reproducible** from the committed code (DISCREPANCY — since **restored**: pinned-convention generator committed and the grid re-run at 8 seeds; restored numbers below).
...
**Benchmark 5 — a parameter-free measurement-wall law** (`results/RESULTS_LAW.md`). Across 150 cells (5 tasks × 6 architectures × 5 shot budgets), a shot-noise-limited QRC classifier's accuracy is predicted *before any noisy run* from three noiseless quantities — readout direction, per-sample decision margins, and the exact multinomial shot-noise covariance projected onto the readout — with zero fitted parameters. **Restored headline (2026-07-04, from committed code with a pinned convention and 8 documented sampling seeds): R² = 0.939 (0.944 noise-corrected), MAE 3.3 pp, bias −1.0 pp** (`src/qrc_law_predict.py` + `src/qrc_law_rerun.py`, raw per-seed cells in `results/law_rerun.json`; per-budget R² 0.86 @250 → 0.96 @64k). The originally reported R² = 0.991 / MAE 1.3 pp did not reproduce from the committed code (see [`AUDITS.md`](AUDITS.md): the prediction-generator script was missing and part of the published observations' provenance is unclear); the 8-seed observation noise floor is 0.9 pp MAE, so the remaining ~3 pp is genuine law error, not sampling noise. The law itself — a zero-parameter closed-form forecast — survives clearly, at honest precision.

**Benchmark 6 — a device-fidelity factor** (`results/RESULTS_GATENOISE.md`). The law is extended from shot noise to *gate* noise. A global depolarizing channel contracts every decision margin by a noiselessly-computable factor `c(γ)`, so gate noise acts, to leading order, as a pure effective-shot reduction: rescaling the shot axis by `c(γ)²` collapses every gate-noisy accuracy curve back onto the noiseless one (420 cells, collapse R² = 0.927 vs 0.851 for ignoring gate noise, advantage growing with noise strength). A QRC practitioner can now estimate the hardware shot budget a classification task needs — accounting for both sampling and gate fidelity — before spending any quantum time. Honest residual: the collapse is ~3 pp, not exact, so gate noise is *approximately* but not perfectly "just fewer shots." (Not yet re-audited post-B5; B6 quotes its own statistics.) ![Gate-noise collapse](figures/qrc_gate_noise.png)
