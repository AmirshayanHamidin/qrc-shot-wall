...
Two task families tested (binary parity/NARMA and continuous Mackey-Glass; B11) and a topology sweep (B12, four coupling graphs), but still 5–6 qubits, one injection scheme, sampling plus the gate-noise channels of B6–B8 — real hardware adds crosstalk and drift, so the wall here is *optimistic*. B12's clean matched-accuracy comparison is in parity's maximal-headroom regime (exact = 1.00); the small-margin Mackey-Glass regime is queued. Feature-count-matched (not wall-clock-matched) classical comparisons. Details and seeds in the write-ups.
