...
**Benchmark 5 — a parameter-free measurement-wall law** (`results/RESULTS_LAW.md`). Across 150 cells (5 tasks × 6 architectures × 5 shot budgets), a shot-noise-limited QRC classifier's accuracy is predicted *before any noisy run* from three noiseless quantities — readout direction, per-sample decision margins, and the exact multinomial shot-noise covariance projected onto the readout — with zero fitted parameters. **Audited precision (see [`AUDITS.md`](AUDITS.md), 2026-07-04): R² = 0.92 (≈0.94 after correcting for observation sampling noise), MAE 3.6 pp.** The originally reported R² = 0.991 / MAE 1.3 pp did not reproduce from the committed code (the prediction-generator script is missing and part of the published observations' provenance is unclear); restoring the generator and a ≥8-seed re-run are queued. The law itself — a zero-parameter closed-form forecast — survives the audit clearly.
